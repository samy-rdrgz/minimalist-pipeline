...
Toute détection (fichier daté, save sur un stable...) construit le même objet — titre, message, sévérité, liste de choix (label + callback, éventuellement un tooltip) — affiché par un unique opérateur popup. Pas de UI ad-hoc par fonctionnalité : ajouter une nouvelle détection ne demande pas de nouveau code d'affichage. Exception : la proposition de version à la première ouverture du jour utilise toujours le dialogue de confirmation natif de Blender à la place de ce mécanisme partagé, parce que sa propre logique n'est pas un simple appel délégué que les choix de la popup partagée pourraient juste envelopper. La notice lecture-seule à l'ouverture déclenchait aussi cette popup autrefois ; maintenant elle est silencieuse pour les raisons qui sont une propriété fixe du fichier — l'indicateur READ-ONLY de la barre du haut porte la raison à la demande à la place (voir [Interface](interface_fr.md)) — et ne s'affiche que quand c'est une information réellement en direct (un autre poste a le fichier ouvert en ce moment).
