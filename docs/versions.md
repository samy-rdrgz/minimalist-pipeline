...
- **Opening a read-only file** (`-stable`, `always_read_only`, or already locked by another machine) shows a popup right away — continue read-only, or increment out of it immediately, rather than finding out only when you try to save.
