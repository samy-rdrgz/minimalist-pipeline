...
- **Ouverture d'un fichier en lecture seule** (`-stable`, `always_read_only`, ou déjà verrouillé par un autre poste) : une popup s'affiche immédiatement — continuer en lecture seule, ou incrémenter tout de suite plutôt que de le découvrir seulement au moment de sauvegarder.
