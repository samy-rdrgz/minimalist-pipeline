...
- **Incrémenter / marquer stable** : même opérateur (`m_pipeline.increment_version`), avec ou sans tag. La popup pré-coche "travaillé cette session" avec ce qui est déjà coché dans le panneau asset/shot (voir [Sessions et verrouillage](sessions-and-locking_fr.md)), et les départements probablement finis (validés au dernier stable, non retouchés depuis). La sidebar masque "Mark as stable" quand le fichier ouvert est déjà la version stable — le retagger stable ne servirait à rien.
